...
For now, a few important things to know:
...
non-edges, or not known whether there is an edge or not an edge respectively. 
Vertices can also have .max_degree or .color properties, though they are by 
default initialized to None. 
...
added to the edge list of one vertex, the corresponding lists of other vertices 
will be updated appropriately. It is best to run this after adding edges to the 
graph.
...
subgraph from the list is present, and False otherwise. The algorithms for this
method generates a prefix tree object 'trie', then performs a depth first 
search with the prefix tree to efficiently search for all forbidden subgraphs
simultaneously. The 50,000 most recent searches for each forbidden subgraph
list are stored in an lru cache 'graph_hash'.
...
graph, adding any forced edges/non_edges along the way. Will return True if in 
the process of updating the graph, a contradiction is forced, otherwise will 
return False and the graph will be updated in place.


.update_graph2_2(subgraph_list): Heavier and slower version of the update_graph function that does a deeper search for forced structure, but can solve more cases. Return type is the same. 
...
Here's a breakdown of the color_argument algorithm (return True means contradiction):

takes in: current vertex v_0, done_list, current_distance, max_distance
...
Supposing neither of these is the case, for each color c_0 in color_list:
    ...
    make 5 lists:
        ...
        list0: Check all the neighbors of v_0 not in done_list. If v_1 in N(v_0) is colored with c_0, then append v_1 to list0. 

        list1: Check all the neighbors of v_0 not in done_list again. If v_1 in N(v_0) is colored with 'None', check if it has any neighbors colored with c_0. If not, make a copy of the graph G', and within G' color v_1 with c_0. Run the methods that extend colorings appropriately through the graph, and check for forks or triangles. If these methods indicate no contradiction, then append v_1 to list1. 

        list2: Check all vertices in the 'unknown' list of v_0 not in done_list. If v_1 in the 'unknown' list of v_0 has no neighbors colored c_0, make a copy of the graph G', and within G' color v_1 with c_0 and add an edge from v_0 to v_1. Run the methods that extend colorings appropriately through the graph, and check for forks or triangles. If these methods indicate no contradiction, then append v_1 to list2.

        list3: If v_0 is not max degree, then make a copy G' of the graph. add a new vertex v_1 to G' and color it with c_0 and add an edge between v_1 and v_0. Run the methods that extend colorings appropriately through the graph, and check for forks or triangles. If these methods indicate no contradiction, then append v_1 to list3.

        list4: Check all vertices in the 'unknown' list of v_0 not in done_list again. If v_1 in the 'unknown' list of v_0 is colored with c_0, make a copy of the graph G', and within G' add an edge from v_0 to v_1. Run the methods that extend colorings appropriately through the graph, and check for forks or triangles. If these methods indicate no contradiction, then append v_1 to list4.
        
        list5: Check the vertices in 

...
max_hash_size2 = 10000  # size for edge_list_hash and subgraph_hash
...
class Trie:
    ...
    def add_subgraph(self, subgraph):
        ...
        """
        temp = self.root
        for i in range(len(subgraph.vertex_list)):
            vertex = subgraph.vertex_list[i]
            ...
            if i == len(subgraph.vertex_list) - 1:
                temp.is_end = True
    ...
    def show(self):
        """Displays all the keys of all the nodes in the trie.
        """
...
class ETrie:
    ...
    the purposes of producing edge lists.
    
    Attributes:
        root (TrieNode): First node in the trie.
        non_edge_list (list[frozenset(int)]): List of non-edges added to the
            graph in order to produce a forbidden subgraph (so far).
    """
    ...
    def initialize(self, subgraph_list):
        """Adds a subgraph from a subgraph List to an etrie object.
    ...
    def show(self):
        """Displays all the keys of all the nodes in the etrie.
        """
        queue = collections.deque([(self.root, 0)])
        while queue:
            popped = queue.popleft()
...
class Vert:
    ...
    Attributes:
        ...
        non_edges: list of integers corresponding to indices of vertices this 
            vertex is known to not have edges with.
...
class Graph:
    ...
    def contains_subgraphs(self, subgraph_list):
        ...
        global trie_list
        if subgraphs_key in trie_list:
            ttrie = trie_list[subgraphs_key]
        else:
            ttrie = Trie()
            ttrie.initialize(subgraph_list)
            trie_list[subgraphs_key] = ttrie
    ...
    def get_edgelists(self, subgraph_list):
        ...
        for subgraph in subgraph_list:
            subgraphs_key.append(tuple([
                ...
                for v in subgraph.vertex_list]))
        subgraphs_key = tuple(subgraphs_key)
        if subgraphs_key not in edge_list_hash:
            edge_list_hash[subgraphs_key] = collections.OrderedDict()
        graph_key = tuple([
            (frozenset(vertex.edges), frozenset(vertex.non_edges))
            for vertex in self.vertex_list])

        # Cache manipulations
        if graph_key in edge_list_hash[subgraphs_key]:
            edge_list_hash[subgraphs_key].move_to_end(graph_key)
            return edge_list_hash[subgraphs_key][graph_key]
        elif len(edge_list_hash[subgraphs_key]) == max_hash_size2:
            edge_list_hash[subgraphs_key].popitem(last=False)

        # Set up prefix tree
        global etrie_list
        if subgraphs_key in etrie_list:
            ttrie = etrie_list[subgraphs_key]
        else:
            ttrie = ETrie()
            ttrie.initialize(subgraph_list)
            etrie_list[subgraphs_key] = ttrie
        ...
        edge_list_hash[subgraphs_key][graph_key] = out
        edge_list_hash[subgraphs_key].move_to_end(graph_key)
        return out
    ...
    def color_vert(self, start_vert):
        ...
        if (len(color_list) == 3) and (not v0.color_start):
            # print('Contradiction!')
            return True
        if (len(color_list) == 2) and (not v0.color_start):
            if v0.color is None:
                v0.color = list({0, 1, 2} - set(color_list))[0]
            elif v0.color != list({0, 1, 2} - set(color_list))[0]:
                return True
        ...
        if v0.color_start:
            if ((v0.max_degree == 3) and len(v0.edges) == 3 and
                    (all([w.color is None for w in self.vertex_list]))):
                i = 0
                for neighbor in v0.edges:
    ...
    def fill_in_colors(self, start_vert=None):
        ...
        possible, running .color_vert() on all the vertices in the graph.
        ...
        while True:
            ...
            for i, edge_list in enumerate(edge_lists):
                ...
                if len(graph_list) == 0:
                    ...
                    for v in self.vertex_list:
                        # Update colors 
                        if with_colors and v.color is None:
                            v.clist = []
                            for tgraph in graph_list:
                                v_temp = tgraph.vertex_list[v.index]
                                if v_temp.color is None:
                                    v.clist.append(-1)
                                else:
    ...
    def update_graph2_2(self, subgraph_list, max_depth=1, verbose=False,
        ...
        while True:
            ...
            if self.update_graph2(subgraph_list, start_vert=None,
                ...
                return True
            # At this point, there is no specified min C_5, we are done.
            if not min_subgraph or self.depth2 >= max_depth:
                return False
            ...
            for tset in subgraphs:
                ...
                for v in self.vertex_list:
                    # Update colors 
                    if with_colors and not v.color:
                        v.clist = []
                        for tgraph in graph_list:
                            v_temp = tgraph.vertex_list[v.index]
                            if not v_temp.color:
    ...
    def update_graph3(self, subgraph_list, verbose=False, with_colors=True,
        ...
        # is updated, vertices can be visited in a cyclical pattern.
        stored_index = 0
    ...
    def update_graph4(self, subgraph_list, verbose=False, with_colors=True,
        ...
        containment (and the assumption that the graph is a minimal
        ...
        while True:
            ...
            if min_subgraph and not exit_loop:
                # Setup
                min_indices = [i for i in range(len(min_subgraph.vertex_list))]
                subgraphs = self.get_subgraph_list(min_subgraph)
                edge_lists = self.get_edgelists(subgraph_list)
            ...
            for v1 in self.vertex_list:
                ...
                for v2 in self.vertex_list:
                    ...
                    if (v1.index != v2.index and len(v1.unknown) == 0 and
                        ...
                        if (not v1.max_degree or (v1.max_degree and
    ...
    def update_graph_flexible(self, subgraph_list, update_speed, verbose=False,
                              min_subgraph=None, with_colors=True, max_depth=1,
                              max_new_vertices=None):
    ...
    def update_graph5(self, subgraph_list, add_vertices=True,
        ...
        while True:
            ...
            for v1, v2, v3 in permutations(self.vertex_list, 3):
                ...
                if not v1.max_degree or len(v1.edges) < v1.max_degree:
                    tgraph = copy.deepcopy(self)
                    new_idx = len(tgraph.vertex_list)
    ...
    def color_argument(self, subgraph_list, start_vert, done_list,
        ...
        4, then the algorithm recurses on that vertex, with (start_vert, i)
        ...
        color_list = list({0, 1, 2} - {start_vertex.color})
        ...
        for tcolor in color_list:
            ...
            for e_idx in start_vertex.edges:
                v1 = self.vertex_list[e_idx]
                if (v1.color is None) and (v1.index not in done_list2):
            ...
            for e_idx in start_vertex.unknown:
                v1 = self.vertex_list[e_idx]
                if (not v1.color) and (v1.index not in done_list2):
...
for k in range(5):
    ...
    v_3 = Vert([4, 2], [], 3)
    v_4 = Vert([0, 3], [], 4)
    v_5 = Vert([k, (2 + k) % 5, 7], [], 5)
    v_6 = Vert([k, (3 + k) % 5], [7], 6)
    v_7 = Vert([5], [0, 1, 2, 3, 4], 7)
    lem3_4_graph = Graph([v_0, v_1, v_2, v_3, v_4, v_5, v_6, v_7])
    lem3_4_graph.clean()
    lem3_4_graph.update_graph(lem2_5_list)
...
for k in range(5):
    ...
    for m in range(5):
        lem3_5_graph.vertex_list[m].fixed_index = True
...
for k in range(5):
    ...
    for m in range(5):
        lem3_6_graph.vertex_list[m].fixed_index = True
...
for k in range(1, 5):
    ...
    for m in range(5):
        lem3_7_graph.vertex_list[m].fixed_index = True
